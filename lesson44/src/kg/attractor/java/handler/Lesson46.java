...
public class Lesson46 extends Lesson45 {

    private static final Map<String, String> sessions = new HashMap<>();
    private static final List<Book> books = new BooksDataModel().getBooks();
    ...
    private String getAuthEmail(HttpExchange exchange) {
    ...
    private void profileGetAuthHandler(HttpExchange exchange) {
    ...
    private void booksGetAuthHandler(HttpExchange exchange) {
        String email = getAuthEmail(exchange);
    ...
    private void borrowBookHandler(HttpExchange exchange) {
        ...
        if(bookIdStr == null) { redirect303(exchange, "/books"); return; }
        int bookId = Integer.parseInt(bookIdStr);

        int currentBooksCount = userBooksCount.getOrDefault(email, 0);

        if (currentBooksCount < 2) {
            for (Book book : books) {
                if (book.getId() == bookId && "AVAILABLE".equals(book.getStatus())) {
                    book.setStatus("BORROWED");
                    userBooksCount.put(email, currentBooksCount + 1);
                    borrowedBooks.put(bookIdStr, email);
                    break;
                }
            }
        }
